...
_EXAMPLES_ROOT = _SCRIPT_PATH.parents[2]
_TABARENA_PKG_ROOT = _SCRIPT_PATH.parents[2] / "tabarena"
for _path in (_EXAMPLES_ROOT, _TABARENA_PKG_ROOT):
...
if __name__ == '__main__':
    ...
    hpo_trajectory_realtabpfn = pd.read_parquet(f"{parquet_path}/RealTabPFN-2.5.parquet")

    hpo_trajectory_lightgbm = pd.read_parquet(f"{parquet_path}/LightGBM.parquet")

    hpo_trajectory_tabm = pd.read_parquet(f"{parquet_path}/TabM.parquet")

    hpo_trajectory_linear = pd.read_parquet(f"{parquet_path}/Linear.parquet")
